        ...
        if (evnt.detail.key == "query") {
          if (evnt.detail.value.length > 2) {
            var products = state.products.filter(function (p) {
              return p.name
                .toLowerCase()
                .includes(evnt.detail.value.toLowerCase());
            });
            listElm.innerHtml = "";
      ...
      domElm.addEventListener("click", function (evnt) {
        ...
        if (elm.getAttribute("data-url")) {
          document.location.hash =
            "#!/products/" + elm.getAttribute("data-url");
          listElm.innerHTML = "";
        }
      });
      // Event triggers
      domElm.querySelector("#query").addEventListener("keyup", function (evnt) {
        evnt.preventDefault();
        // Change state
        setState("query", evnt.target.value.trim());
      });
      // Toggle bottom border on focus
      domElm.querySelector("#query").addEventListener("focus", function (evnt) {
        evnt.target.style = "border-bottom: 1px solid rgb(237, 100, 47);";
      });
      // Toggle bottom border on blur
      domElm.querySelector("#query").addEventListener("blur", function (evnt) {
